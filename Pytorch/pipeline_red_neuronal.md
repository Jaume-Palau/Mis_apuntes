...
| `from torch.utils.data import DataLoader, TensorDataset` | `TensorDataset` agrupa tensores de entrada y salida en un dataset, y `DataLoader` los divide en lotes para entrenar por batches. | `train_loader = DataLoader(TensorDataset(X_train, y_train), batch_size=32, shuffle=True)` |
