...
| `RIGHT JOIN`      | Todas las filas de la derecha + coincidencias de la izquierda (`NULL` si no coincide) | Igual que `LEFT` pero prioriza la tabla de la derecha          | `SELECT ... FROM t1 RIGHT JOIN t2 ON t1.col = t2.col;`   |
| `FULL OUTER JOIN` | Todas las filas de ambas tablas, coincidan o no                                       | MySQL no lo soporta (se suele simular)                         | `LEFT JOIN` + `UNION` + `RIGHT JOIN`                     |
...
| `IN`              | `TRUE` si el valor existe en una lista o subconsulta                                  | Subconsultas simples                                           | `WHERE t1.col IN (SELECT col FROM t2);`                  |
| `EXISTS`          | `TRUE` si la subconsulta devuelve filas                                               | Subconsultas correlacionadas, suele ser eficiente              | `WHERE EXISTS (SELECT 1 FROM t2 WHERE t2.col = t1.col);` |
...
| `HASH`           | `-- En InnoDB no se crea manualmente; en MEMORY puede usarse HASH` | Búsquedas por igualdad (`=`)                                  | InnoDB no soporta `USING HASH` (HASH es típico de MEMORY)                                            |
| `FULLTEXT`       | `CREATE FULLTEXT INDEX idx_texto ON tabla(col1, col2);`            | Búsqueda de texto largo                                       | Se consulta con `MATCH(col1, col2) AGAINST('palabras' IN BOOLEAN MODE);` (operadores como `+` y `-`) |
